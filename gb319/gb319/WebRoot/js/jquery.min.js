...
(function() {
	...
	jQuery.fn = jQuery.prototype = {
			...
			if (selector.nodeType) {
				...
				if (match && (match[1] || !context)) {
					if (match[1])
						...
						if (elem)
							if (elem.id != match[3]) return jQuery().find(selector);else {
								this[0] = elem;
								this.length = 1;return this;
						}
						else
		...
		attr : function(name, value, type) {
			...
			if (name.constructor == String)
				if (value == undefined) return this.length && jQuery[type || "attr"](this[0], name) || undefined;else {
					options = {};
					options[name] = value;
			}
	...
	jQuery.extend({
		...
		css : function(elem, name, force) {
			if (name == "width" || name == "height") {
				...
				function getWH() {
					val = name == "width" ? elem.offsetWidth : elem.offsetHeight;var padding = 0,
						border = 0;
		...
		curCSS : function(elem, name, force) {
			...
			if (name == "opacity" && jQuery.browser.msie) {
				ret = jQuery.attr(elem.style, "opacity");return ret == "" ? "1" : ret;
			}
			...
			if (!force && elem.style && elem.style[name])
				...
				if (getComputedStyle && !color(elem))
					ret = getComputedStyle.getPropertyValue(name);else {
					var swap = [],
						stack = [];
	...
	jQuery.extend({
		...
		find : function(t, context) {
			...
			while (t && last != t) {
				var r = [];
				last = t;
				t = jQuery.trim(t);var foundToken = false;
				var re = quickChild;
				var m = re.exec(t);
				if (m) {
			...
			done = jQuery.merge(done, ret);return done;
		},
	...
	jQuery.event = {
		add : function(elem, types, handler, data) {
			...
			var events = jQuery.data(elem, "events") || jQuery.data(elem, "events", {}),
				handle = jQuery.data(elem, "handle") || jQuery.data(elem, "handle", function() {
					...
					val = jQuery.event.handle.apply(arguments.callee.elem, arguments);return val;
				});
			handle.elem = elem;jQuery.each(types.split(/\s+/), function(index, type) {
		...
		remove : function(elem, types, handler) {
			...
			if (events) {
				if (types == undefined || (typeof types == "string" && types.charAt(0) == "."))
					for (var type in events) this.remove(elem, type + (types || ""));else {
					if (types.type) {
				...
				if (!ret) {
					...
					if (handle)
						handle.elem = null;
					jQuery.removeData(elem, "events");jQuery.removeData(elem, "handle");
				}
			}
		},
		...
		special : {
			...
			mouseenter : {
				setup : function() {
					...
					jQuery(this).bind("mouseover", jQuery.event.special.mouseenter.handler);return true;
				},
				teardown : function() {
					if (jQuery.browser.msie) return false;
					jQuery(this).unbind("mouseover", jQuery.event.special.mouseenter.handler);return true;
				},
			...
			mouseleave : {
				...
				teardown : function() {
					if (jQuery.browser.msie) return false;
					jQuery(this).unbind("mouseout", jQuery.event.special.mouseleave.handler);return true;
				},
				handler : function(event) {
					if (withinElement(event, this)) return true;
		...
		one : function(type, data, fn) {
			return this.each(function() {
				jQuery.event.add(this, type, function(event) {
					jQuery(this).unbind(event);return (fn || data).apply(this, arguments);
				}, fn && data);
			});
		},
		...
		toggle : function() {
			...
			return this.click(function(event) {
				this.lastToggle = 0 == this.lastToggle ? 1 : 0;event.preventDefault();return args[this.lastToggle].apply(this, arguments) || false;
			});
		},
	...
	jQuery.extend({
		...
		ajax : function(s) {
			...
			xml.open(s.type, s.url, s.async, s.username, s.password);try {
	...
	jQuery.fx.prototype = {
		...
		custom : function(from, to, unit) {
			...
			this.now = this.start;
			this.pos = this.state = 0;this.update();
			var self = this;
			function t(gotoEnd) {
				return self.step(gotoEnd);
			}
			t.elem = this.elem;jQuery.timers.push(t);
		...
		show : function() {
			...
			this.options.show = true;this.custom(0, this.cur());
	...
	jQuery.fn.offset = function() {
		var left = 0,
			...
			elem = this[0],
			results;
		if (elem) with(jQuery.browser){var parent = elem.parentNode,
				offsetChild = elem,
			...
			if (elem.getBoundingClientRect) {
				...
				add(box.left + Math.max(doc.documentElement.scrollLeft, doc.body.scrollLeft), box.top + Math.max(doc.documentElement.scrollTop, doc.body.scrollTop));add(-doc.documentElement.clientLeft, -doc.documentElement.clientTop);
			} else {
